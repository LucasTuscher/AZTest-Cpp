...
Eine Fixture ist eine Klasse, die von `AZTest::TestFixture` erbt. In dieser Klasse können Sie die folgenden virtuellen Methoden überschreiben:
...
- **`TearDown()`:** Diese Methode wird **nach jedem Test** aufgerufen, der die Fixture verwendet. Hier räumen Sie Ressourcen auf (z.B. Speicher freigeben, Verbindungen schließen).
...
Stellen Sie sich vor, Sie haben eine `Datenbank`-Klasse und mehrere Tests müssen mit einer verbundenen Datenbankinstanz arbeiten.
...
class DatenbankFixture : public AZTest::TestFixture {
protected:
