...
void TestRegistry::RecordFailure(const std::string& msg, const char* file, int line) {
    ...
    currentFailureMsg_ = GetTraceStack() + msg;
    currentFile_ = file;
    currentLine_ = line;
}
...
int TestRegistry::RunAllTests() {
    ...
    std::function<void()> currentSuiteTearDown;
    ...
    for (int iteration = 0; iteration < repeatCount_ && !aborted; ++iteration) {
        if (shuffle_) {
            ...
            std::shuffle(runnable.begin(), runnable.end(), rng);
        }

        for (const auto& test : runnable) {
            // Handle suite changes
            if (test.suite != currentSuite) {
                // Tear down previous suite
                if (suiteSetUpCalled && currentSuiteTearDown) {
                    currentSuiteTearDown();
                }

                if (!currentSuite.empty()) {
                    for (auto& reporter : reporters_) {
                        reporter->OnTestSuiteEnd(currentSuite);
                    }
                }
                ...
                for (auto& reporter : reporters_) {
                    reporter->OnTestSuiteStart(currentSuite);
                }

                // Set up new suite if it's a fixture test
                if (test.isFixture && test.setUpFunc) {
                    test.setUpFunc();
                    suiteSetUpCalled = true;
                    currentSuiteTearDown = test.tearDownFunc;
                } else {
                    suiteSetUpCalled = false;
                    currentSuiteTearDown = nullptr;
                }
            }
            ...
            if (currentTestSkipped_) {
                ...
                result.failureMessage = currentFailureMsg_;
                result.fileName = currentFile_.empty() ? test.file : currentFile_;
                result.lineNumber = currentLine_ == 0 ? test.line : currentLine_;
            } else {
